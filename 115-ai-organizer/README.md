...
详细过程、当前断点和换电脑恢复步骤，见 [2026-09-03 交接文档](docs/HANDOFF_2026-09-03.md)（最新）和 [2026-09-02 交接文档](docs/HANDOFF_2026-09-02.md)。第一阶段排障原始记录保留在 [进度.md](进度.md)。

**2026-09-03：整理目标已从“云下载”切换为“星标视频”。** OpenList 存储（115 Open 驱动）的根文件夹 ID 已改为星标视频（CID `3508627805157983969`），挂载路径 `/星标视频`；只读账号 `organizer-readonly` 基本路径同步更新；项目 `.env` 的 `OPENLIST_MOUNT_PATH`、`ALLOWED_ROOT`、`DEFAULT_SCAN_DIR` 均改为 `/星标视频`。115 授权当日重新做过一次（旧 refresh_token 失效）。

切换后完成完整只读扫描：422 个目录、8860 个文件、约 277GB，全部取得 115 原生 ID，无 SHA1 重复风险组。本轮批准数为 0，真实 115 写操作数为 0。

首次实盘验收（2026-09-02，历史记录）：在“星标视频”中选择 1 个 8.46MB 推广样本，成功新建 `已整理/首次实测`、改名为 `推广样本_01.mp4` 并移动；按同一原生 ID 复核目标存在、来源消失、大小不变。

## 整理规则（2026-09-03 起）

`rebuild-plans` 分三遍生成建议：

1. **逐文件分类**：扩展名 + 文件名/路径关键词，识别电影、电视剧、动漫、纪录片、综艺、字幕、图片等。电影按 `/电影/<片名 (年份)>/` 归档；动漫按 `/动漫/<番剧名>/Season N/`（番剧名自动去掉字幕组标签）。
2. **系列归集**：文件名去掉集数/编号/字幕组标签/版本词后计算“系列键”，同一键且 ≥2 个文件的视频归入 `/系列/<系列名>/`，不改动文件名。同目录或 ≥4 个成员的组为高置信度；跨目录小组为中置信度。图片文件名能对上视频系列的跟随该系列，对不上的留在 `/图片/`。
3. **时间兜底**：仍无法识别的视频按 115 添加时间（月）归入 `/按时间归集/YYYY-MM/`，低置信度，需人工核对。

安全边界不变：图片和低置信度项不会被“批准安全候选”自动批准；所有写入仍需人工批准 + 防篡改操作清单 + 确认码。

## 网页分页

整理计划列表分页显示（每页 50/200/1000 条可选），避免一次渲染上万条导致页面长时间停留在骨架屏。“批准筛选结果”批准的是当前筛选条件下的**全部**条目，不只是当前页。
