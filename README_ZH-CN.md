...
- **GLM 促销雷达（M23）**：只读展示 Docker 常驻扫描智谱官网、去重后发钉钉、无新优惠显示无；线上页面不执行监控。
...
### 23. 📡 GLM 促销雷达
- **场景**：展示 Docker 里的智谱促销雷达有什么、做了什么：五次扫描、官方渠道、钉钉推送、无新优惠显示无。
- **边界**：页面不联网、不启容器、不读取本机状态、不发钉钉；真实监控保留在独立仓库 glm-monitor。
