...
def _denied_rows(path, denied):
    """Keep unreadable descendants visible even below the size/Top-N filters."""
    key = _path_key(path)
    prefix = key.rstrip(os.sep) + os.sep
    return [{"name": "(permission denied)", "path": failed,
             "size_kb": 0, "size_h": "?", "denied": True}
            for failed_key, failed in sorted(denied.items())
            if failed_key == key or failed_key.startswith(prefix)]


def dir_size_bytes(path, cache=None, denied=None):
    """Recursive byte count; optional state belongs to one scan only."""
    if cache is None:
        cache = {}
    if denied is None:
        denied = {}
            ...
            for e in it:
                try:
                    ...
                    if e.is_file(follow_symlinks=False):
                        ...
                        total += dir_size_bytes(e.path, cache, denied)
                except (PermissionError, OSError):
                    denied[_path_key(e.path)] = e.path
    except (PermissionError, OSError):
        denied[key] = path
    cache[key] = total
    return total
...
def scandir_children(path, min_kb=51200, limit=40, cache=None, denied=None):
    ...
    try:
        ...
        denied[_path_key(path)] = path
        return _denied_rows(path, denied)
    for name in entries:
        child = os.path.join(path, name)
        if os.path.islink(child):
            continue
        try:
            kb = (os.path.getsize(child) if os.path.isfile(child)
                  else dir_size_bytes(child, cache, denied)) // 1024
        except (PermissionError, OSError):
            denied[_path_key(child)] = child
            continue
        if kb < min_kb:
            continue
        results.append({"name": name, "path": child, "size_kb": kb, "size_h": human(kb)})
    results.sort(key=lambda r: r["size_kb"], reverse=True)
    return results[:limit] + _denied_rows(path, denied)
...
def scan_windows():
    ...
    groups = {}
    # Shared by overlapping groups, discarded before the next scan.
    size_cache, denied = {}, {}
    for key, path, floor in targets:
        groups[key] = scandir_children(path, min_kb=floor,
                                       cache=size_cache, denied=denied)
    ...
    for path in dev_paths:
        if not os.path.isdir(path):
            continue
        try:
            kb = dir_size_bytes(path, size_cache, denied) // 1024
        except (PermissionError, OSError):
            continue
        if kb >= 51200:
            dev.append({"name": os.path.basename(path.rstrip("\\/")) or path,
                        "path": path, "size_kb": kb, "size_h": human(kb)})
        dev.extend(_denied_rows(path, denied))
