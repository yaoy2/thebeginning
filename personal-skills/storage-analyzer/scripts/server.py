...
def best_effort_delete(path, mode):
    """Delete one allowlisted root, skipping locked descendants when needed.

    The fast path preserves the old behavior.  If a directory operation fails
    (commonly WinError 32 on Windows), split it into children and continue so a
    single busy cache file cannot block the rest of a large temp directory.
    """
    removed = []
    failed = []

    def remove_one(target):
        ...
        try:
            ...
            if not os.path.isdir(target) or os.path.islink(target):
                failed.append({"path": target, "error": str(first_error)})
                return

        # The directory itself could not be processed.  Retry child-by-child;
        # recursively split only the children that are also busy directories.
        try:
        ...
        for child in children:
            remove_one(child.path)
        try:
            os.rmdir(target)  # empty only; content was trashed/deleted above
            removed.append(target)
        except Exception as e:
    ...
    return {
        "path": path,
        "status": "complete" if not exists else ("partial" if removed else "failed"),
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if allow is None:
            ...
            return
        requested = req.get("paths") or []
        validated = []
        # Validate the complete request before any destructive operation.  A
        # bad path must never produce a half-applied batch.
        for p in requested:
            rp = expand(p)
            if rp not in allow:
            ...
            if not any(rp == base or rp.startswith(base + os.sep) for base in roots):
                ...
                return
            validated.append((p, rp))

        done = []
        results = []
        for p, rp in validated:
            try:
                if mode == "open":
                    open_in_file_manager(rp)
                    result = {"path": p, "status": "complete", "removed_count": 0, "failed": []}
                else:
                    result = best_effort_delete(rp, mode)
                    result["path"] = p
                results.append(result)
                if result["status"] == "complete":
                    done.append(p)
            except Exception as e:
                results.append({"path": p, "status": "failed", "removed_count": 0,
                                "failed": [{"path": p, "error": str(e)}]})
        failures = [f for result in results for f in result.get("failed", [])]
        partial = any(result.get("status") == "partial" for result in results)
        self._send(200, json.dumps({"ok": not failures, "partial": partial,
                                   "done": done, "results": results,
                                   "failed": failures}, ensure_ascii=False))
