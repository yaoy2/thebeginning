...
class HomePageTest(unittest.TestCase):
    def test_home_page_uses_command_center_copy_and_pagination(self):
        ...
        self.assertIn('st.button(tool["title"], key=f"open_{tool[\'code\']}")', page_source)
        self.assertIn('st.switch_page(tool["page"])', page_source)
        self.assertIn("tool-title-link", page_source)
        self.assertIn("tool-title-ghost", page_source)
