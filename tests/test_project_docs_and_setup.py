...
def test_readmes_cover_current_grade_workbench_modules():
    ...
    assert "十七个现存工具模块" in chinese
    assert "M19" not in chinese and "M18" in chinese and "M17" in chinese and "M16" in chinese
    assert "Seventeen Current Tool Modules" in english
    assert "M19" not in english and "M18" in english and "M17" in english and "M16" in english
